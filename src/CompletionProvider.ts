...
export class MessageKeyCompletionProvider implements vscode.CompletionItemProvider {
    async provideCompletionItems(document: vscode.TextDocument, position: vscode.Position) {
        const config = vscode.workspace.getConfiguration('java-message-key-navigator');
        const patterns: string[] | undefined = config.get('messageKeyExtractionPatterns');
        if (!patterns || patterns.length === 0) return undefined;

        const lineText = document.lineAt(position).text;

        // 行にパターンを含むかだけ判定する
        const matchesPattern = patterns.some(pattern => lineText.includes(pattern));
        if (!matchesPattern) return undefined;

        // 入力中の文字を取得
        const lineUntilPosition = document.lineAt(position).text.substring(0, position.character);
...
function generateCompletionItems(input: string): vscode.CompletionItem[] {
    ...
    return filteredKeys.map(key => {
        ...
        const item = new vscode.CompletionItem(`${key} - ${value}`, vscode.CompletionItemKind.Value);
        item.insertText = key;
